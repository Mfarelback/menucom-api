...
|--------|--------|-----------|
| **maxCommerces** | ✅ Implementado | `src/commerce/commerce.service.ts` (via `MembershipService`) |
...
**Ubicación:** `src/commerce/commerce.service.ts` (nueva validación inline)

**Cómo funciona:**
- `CommerceService.create()` inyecta `MembershipService` para obtener los límites del plan del usuario
- Compara el conteo actual de commercios del usuario contra `maxCommerces` del plan
...
if (limits.maxCommerces !== undefined && limits.maxCommerces !== -1) {
  const count = await this.commerceRepository.count({ where: { ownerId: userId } });
  if (count >= limits.maxCommerces) {
    throw new BadRequestException('Límite de comercios alcanzado');
  }
}
```
...
1. **3 de 8 límites están validados** (maxCommerces, maxCatalogs, maxCatalogItems)
