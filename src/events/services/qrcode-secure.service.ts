...
export class QRCodeSecureService {
  ...
  constructor() {
    this.secretKey = process.env.TICKET_QR_SECRET || 'default-secret-change-in-production';
   ...
   */
  generateSecureQR(data: Omit<SecureQRData, 'timestamp' | 'signature'>): string {
  ...
  validateSecureQR(qrCode: string): SecureQRData | null {
    try {
      ...
      if (age > this.qrExpirationMs) {
        this.logger.warn(`QR expired for ticket: ${data.ticketId}, age: ${age}ms`);
        return null;
      }
